...
import static org.aarboard.nextcloud.api.utils.NextcloudResponseHelper.getAndWrapException;
...
import lombok.val;
import org.aarboard.nextcloud.api.provisioning.CreateUser;
...
public class TestUserGroupAdmin extends ATestClass {
    ...
    @Test
    public void t02_testCreateUser() {
        System.out.println("createUserV2");
        if (_nc != null) {

            val groupList = new ArrayList<String>();
            groupList.add("user");

            CreateUser u = CreateUser.builder()
                    .userid("uuuuuu6")
                    .password("1uuu1uuu1")
                    .quota("10 GB")
                    .groups(groupList)
                    .email("uuuuuu6@linkzonetel.com")
                    .language("en")
                    .build();

            val answer = _nc.createUserAsync(u);
            val res = getAndWrapException(answer);

            assertTrue(res.getStatusCode() == 100);
        }
    }
